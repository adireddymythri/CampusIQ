...
export function GoogleAuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    async function finish() {
      ...
      if (!codeFromUrl && localStorage.getItem('accessToken') && sessionStorage.getItem(PENDING_KEY)) {
        try {
          await completeLogin()
        } catch {
          await failLogin()
      ...
      try {
        const { data } = await api.post<{
          ...
          accessToken: string
          refreshToken: string
        }>('/auth/google/exchange', { code: exchangeCode })
        ...
        if (localStorage.getItem('accessToken') && sessionStorage.getItem(PENDING_KEY)) {
          try {
            await completeLogin()
            return
          } catch {
