    ...
    return codeParts.map((cPart, cIdx) => {
      if (cPart.startsWith("`") && cPart.endsWith("`")) {
        return (
          <code
            key={`${idx}-${cIdx}`}
            className="bg-slate-950 px-1.5 py-0.5 rounded text-xs font-mono text-indigo-300 border border-white/5"
          >
            {cPart.slice(1, -1)}
          </code>
        );
      ...
      return mathParts.map((mPart, mIdx) => {
        if (mPart.startsWith("$") && mPart.endsWith("$")) {
          return (
            <span
              key={`${idx}-${cIdx}-${mIdx}`}
              className="font-mono text-xs text-indigo-300 bg-indigo-500/10 px-1 py-0.5 rounded border border-indigo-500/20"
            >
              {mPart.slice(1, -1)}
            </span>
          );
      ...
      {parts.map((part, i) => {
        if (part.startsWith("```")) {
          ...
          return (
            <pre
              key={i}
              className="bg-[#050816] p-4 rounded-xl text-xs font-mono my-3 overflow-x-auto text-slate-300 border border-white/5 leading-relaxed"
            >
              <code>{code}</code>
            </pre>
          );
        ...
        return subParts.map((subPart, j) => {
          if (subPart.startsWith("$$")) {
            ...
            return (
              <div
                key={`${i}-${j}`}
                className="text-center my-3 p-3 bg-indigo-500/5 rounded-xl border border-indigo-500/10 font-mono text-sm text-indigo-200"
              >
                {eq}
              </div>
            );
          ...
          return lines.map((line, k) => {
            ...
            if (line.trim().startsWith("* ") || line.trim().startsWith("- ")) {
              ...
              return (
                <li
                  key={`${i}-${j}-${k}`}
                  className="ml-4 list-disc text-sm text-slate-300 my-1"
                >
...
export function AIPage() {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const { data } = await api.get("/notes?limit=100");
    ...
    if (!noteId) {
      ...
      const fetchGeneralHistory = async () => {
        try {
          ...
          if (
            historyRes.data.ok &&
            historyRes.data.conversation?.messages?.length
          ) {
            setMessages(
              historyRes.data.conversation.messages.map((m) => ({
                ...m,
    ...
    const fetchSelectedNote = async () => {
      ...
      try {
        ...
        if (data.ok) {
          ...
          try {
            const historyRes = await api.get(`/ai/history?noteId=${noteId}`);
            if (
              historyRes.data.ok &&
              historyRes.data.conversation?.messages?.length
            ) {
    ...
    try {
      ...
      if (data.ok) {
        ...
        const aiMsg = {
          ...
          timestamp: new Date(),
          meta: {
            usedPdf: data.meta?.hasPdfContent,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Powered by OpenRouter — reads PDFs and searches the web.
              </p>
            </div>
          </div>
              ...
              {notes.length === 0 ? (
                ...
                notes.map((note) => {
                  ...
                  return (
                    ...
                    >
                      <FileText
                        className={`size-4.5 shrink-0 mt-0.5 ${isActive ? "text-indigo-400" : "text-slate-500 group-hover:text-slate-400"}`}
                      />
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {isAI &&
                          msg.meta &&
                          (msg.meta.usedPdf || msg.meta.usedWebSearch) && (
            ...
            {selectedNote && (
              <div className="px-6 py-2 border-t border-white/5 bg-white/[0.01] flex flex-wrap gap-2">
                ...
                <button
                  onClick={() =>
                    handleSendMessage(
                      "Explain the main key formulas and math concepts in this note.",
                    )
                  }
                  disabled={loading}
                ...
                <button
                  onClick={() =>
                    handleSendMessage(
                      "Give me a quick 3-question multiple choice practice quiz based on this note.",
                    )
                  }
                  disabled={loading}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-white/5 bg-white/5 text-xs text-slate-300 hover:bg-white/10 transition-colors disabled:opacity-50"
