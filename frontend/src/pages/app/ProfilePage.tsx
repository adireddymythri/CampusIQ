...
export function ProfilePage() {
  ...
  const computeLevel = (xp: number) => {
    if (xp < 100) return { level: 1, title: 'Novice Learner', min: 0, max: 100 }
  ...
  const getActivityDetails = (act: any) => {
    switch (act.type) {
      case 'quiz_finish':
        return { icon: Award, color: 'text-amber-400', bg: 'bg-amber-400/10', title: 'Completed a Quiz', desc: `Scored ${act.meta?.score}/${act.meta?.total} and earned +${act.meta?.xpEarned} XP` }
      case 'quiz_start':
        return { icon: Target, color: 'text-indigo-400', bg: 'bg-indigo-400/10', title: 'Started a Quiz', desc: 'Initiated a new practice session.' }
      case 'note_upload':
        return { icon: FileUp, color: 'text-emerald-400', bg: 'bg-emerald-400/10', title: 'Uploaded a Document', desc: 'Contributed study material to the platform.' }
      case 'note_view':
        return { icon: BookOpen, color: 'text-blue-400', bg: 'bg-blue-400/10', title: 'Studied a Document', desc: 'Viewed course materials.' }
      default:
        return { icon: Sparkles, color: 'text-slate-400', bg: 'bg-slate-400/10', title: 'Platform Activity', desc: 'Engaged with CampusIQ.' }
    }
  }
  ...
  const progressPercent = Math.min(100, Math.max(0, ((data.profile.xp - lvlInfo.min) / (lvlInfo.max - lvlInfo.min)) * 100))
            ...
            <div className="lg:col-span-4 space-y-6">
                ...
                <div className="relative z-10">
                  <div className="size-24 mx-auto rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 p-[3px] shadow-2xl shadow-indigo-500/20">
                    <div className="size-full rounded-full bg-[#0a0d1d] flex items-center justify-center overflow-hidden border-2 border-[#0a0d1d]">
                      {data.user.avatarUrl ? (
                        <img src={data.user.avatarUrl} alt="Avatar" className="size-full object-cover" />
                      ) : (
                        <span className="text-3xl font-black text-white">{data.user.name.slice(0, 2).toUpperCase()}</span>
                      )}
                    </div>
                  </div>

                  <h2 className="text-xl font-bold text-white mt-5">{data.user.name}</h2>
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mt-1">{data.user.role}</p>

                  <div className="mt-6 pt-6 border-t border-white/5">
                    <div className="flex items-center justify-between text-xs mb-2">
                      <span className="text-slate-400 font-bold">About Me</span>
                      {!isEditingBio && (
                        <button onClick={() => setIsEditingBio(true)} className="text-indigo-400 hover:text-indigo-300 flex items-center gap-1 font-semibold">
                    ...
                    {isEditingBio ? (
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button
                            onClick={() => { setIsEditingBio(false); setBioInput(data.profile.bio || '') }}
                      ...
                      <p className="text-xs text-slate-300 leading-relaxed text-left">
                        {data.profile.bio || "This student hasn't written a bio yet. They are busy studying!"}
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-[#0c112b]/80 border border-white/5 rounded-3xl p-6 shadow-xl backdrop-blur space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-300 font-medium">{data.user.email}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-500 font-semibold">Joined</span>
                  <span className="text-xs text-slate-300 font-medium">{new Date(data.user.createdAt).toLocaleDateString()}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-500 font-semibold">Branch</span>
                  <span className="text-xs text-slate-300 font-medium">{data.profile.branchName || 'Not Set'}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-500 font-semibold">Semester</span>
                  <span className="text-xs text-slate-300 font-medium">{data.profile.semesterName ? `Semester ${data.profile.semesterName}` : 'Not Set'}</span>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-[#0c112b] to-[#12183d] border border-white/5 rounded-3xl p-8 shadow-xl backdrop-blur relative overflow-hidden flex items-center justify-between">
                ...
                <div className="relative z-10 flex-1 pr-8 border-r border-white/10">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-sm font-bold text-amber-400 uppercase tracking-widest">Level {lvlInfo.level}</span>
                  </div>
                  <h2 className="text-3xl font-black text-white">{lvlInfo.title}</h2>
                  
                  <div className="mt-6">
                    <div className="flex justify-between text-xs font-bold text-slate-400 mb-2">
                      <span>{data.profile.xp} XP</span>
                      <span>{lvlInfo.max} XP to Level {lvlInfo.level + 1}</span>
                    </div>
                ...
                <div className="relative z-10 pl-8 flex gap-6">
                  <div className="text-center">
                    ...
                    <span className="block text-2xl font-black text-white">{data.stats.notesUploaded}</span>
                  ...
                  <div className="text-center">
                    ...
                    <span className="block text-2xl font-black text-white">{data.stats.quizzesTaken}</span>
                    <span className="block text-[10px] font-bold text-slate-500 uppercase">Quizzes</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0c112b]/80 border border-white/5 rounded-3xl p-8 shadow-xl backdrop-blur relative overflow-hidden">
                ...
                {data.recentActivity.length === 0 ? (
                  <div className="text-center py-12 border border-white/5 border-dashed rounded-2xl">
                    <p className="text-sm text-slate-500 font-medium">No recent activity found. Start studying!</p>
                  </div>
                ) : (
                    ...
                    {data.recentActivity.map((act) => {
                      ...
                      return (
                        <div key={act._id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                          {/* Icon marker */}
                          <div className={`flex items-center justify-center size-10 rounded-full border-4 border-[#0c112b] shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow-md ${details.bg} ${details.color} z-10`}>
                          ...
                          <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-[#121633] p-4 rounded-2xl border border-white/5 hover:border-white/10 transition-colors shadow-lg">
                            <div className="flex items-center justify-between mb-1">
                              <span className={`text-[10px] font-bold uppercase tracking-wider ${details.color}`}>{details.title}</span>
                              <span className="text-[10px] text-slate-500 font-semibold">{new Date(act.createdAt).toLocaleDateString()}</span>
                            </div>
