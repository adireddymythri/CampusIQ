  ...
  useEffect(() => {
    const fetchNotes = async () => {
      ...
      try {
        ...
        const { data } = await api.get<{ ok: boolean; items: NoteItem[] }>(`/notes?${params.toString()}`)
        if (data.ok) {
        ...
        <div className="relative z-10 p-6 lg:p-8">
          ...
          <header className="flex items-center justify-between pb-6 border-b border-white/5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white tracking-tight">Browse Notes</h1>
                <p className="text-xs text-slate-400 mt-0.5">Filter and access high-quality study materials</p>
              </div>
            </div>
